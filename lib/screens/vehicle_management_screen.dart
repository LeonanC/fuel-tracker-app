...
class VehicleCard extends StatelessWidget {
  ...
  Future<void> _shareCard() async {
    ...
    if(image != null){
      ...
      final imagePath = await File('${directory.path}/${vehicle.nickname}.png').create();
      await imagePath.writeAsBytes(image);
      await Share.shareXFiles([XFile(imagePath.path)], text: 'Meu veículo cadastrado no Fuel Tracker!');
    }
  }
  ...
  Widget _buildVehicleAvatar(VehicleModel vehicle) {
    return CircleAvatar(
      ...
      backgroundImage: (vehicle.imageUrl != null && vehicle.imageUrl!.isNotEmpty)
          ? FileImage(File(vehicle.imageUrl!))
          : null,
      child: (vehicle.imageUrl == null || vehicle.imageUrl!.isEmpty)
          ? Icon(RemixIcons.car_line, color: AppTheme.primaryFuelColor, size: 28)
          : null,
    );
  }
