  ...
  Future<String?> exportAndShareEntries(List<FuelEntryModel> entries) async {
    try {
      ...
      final String fileName =
          'relatorio_abastecimento_${DateTime.now().year}-${DateTime.now().month}-${DateTime.now().day}.csv';
      final path = '${directory.path}/$fileName';
      final file = File(path);
      await file.writeAsString(BOM);

      await Share.shareXFiles([
        XFile(file.path),
      ], text: 'Segue seu relatório de abastecimentos.');
