...
class FuelDb extends DatabaseHelper {
  ...
  Future<int> insertFuel(FuelEntry entry) async {
    final db = await getDb();
    return await db.insert('fuel_entries', entry.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
  }
  ...
  Future<int> insertMaintenance(Map<String, dynamic> values) async {
    final db = await getDb();
    values.remove('id');
    int newId = await db.insert('manutencao', values, conflictAlgorithm: ConflictAlgorithm.replace);
    return newId;
  }
  ...
  Future<int> insertStation(GasStationModel station) async {
    final db = await getDb();
    return await db.insert('gas_stations', station.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
  }
